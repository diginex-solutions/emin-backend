...
async function createTemplate(req, res, next) {
  ...
  const template = await templateSvc.createTemplate(req.body.authed.RESERVED_USID, name, inputs, languages)
  res.json(objReplaceKeyName(template, '_id', 'id'))
}

async function listTemplates(req, res, next) {
  const templates = await templateSvc.listTemplates(req.body.authed.RESERVED_USID)
  res.json(objReplaceKeyName(templates, '_id', 'id'))
}

async function deleteTemplate(req, res, next) {
  const templateId = req.params.templateId
  const template = await templateSvc.deleteTemplate(req.body.authed.RESERVED_USID, templateId)
  res.json(objReplaceKeyName(template, '_id', 'id'))
}
