...
async function createFolder(req, res, next) {
  if (req.body.path) {
    ...
    let [folderRes, errMsg] = await fileSvc.createFolder(req.body.authed.RESERVED_USID, req.body.path, req.time)
    if (errMsg) {
      errors.makeMessage(res, errors.invalidRequestPayload, errMsg)
    } else {
      res.json(resMapper.filesView(folderRes, req.body.authed))
    }
  } else {
...
async function deleteFolder(req, res, next) {
  if (req.body.path) {
    const validated = fileSvc.reservedPathValidation(req.body.path)
    let [folderRes, errMsg] = await fileSvc.deleteFolder(req.body.authed.RESERVED_USID, req.body.path)
    if (errMsg) {
      errors.makeMessage(res, errors.invalidRequestPayload, errMsg)
    } else {
      res.json(resMapper.filesView(folderRes, req.body.authed))
...
let moveOrRename = (fn) => {
  return async (req, res, next) => {
    ...
    let [folderRes, errMsg] = await fn(req.body.authed.RESERVED_USID, documents, pathAfter)
    if (errMsg) {
      errors.makeMessage(res, errors.invalidRequestPayload, errMsg)
    } else {
