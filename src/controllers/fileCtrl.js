...
const resolveBlobName = async (req, file) => {
  ...
  if (currentRoute === documentPath || currentRoute === documentChecklistPath || currentRoute === versionDocumentPath || currentRoute === constants.EXT_AUTH_PATH.documents) {
    return req.multerCustomHeader.RESERVED_USID + '/' + req['trustVersionFileId']
  } else if (currentRoute === caseDocPath) {
    console.log('req.url', req.url)
    // const [myUserCase, caseDb] = await caseSvc.getMyCase(req.body.authed.RESERVED_USID, req.params.caseId)
...
const azureStorage = new MulterAzureStorage({
  ...
  containerAccessLevel: settings.AZURE_BLOB_STORAGE_CONTAINER_ACCESS_LEVEL,
})
...
async function listFiles(req, res, next) {
  const files = await fileSvc.listAllAccesses(req.body.authed.RESERVED_USID)
  res.json(resMapper.filesView(files, req.body.authed))
}

async function listChecklistFiles(req, res, next) {
  const files = await fileSvc.listChecklistAccesses(req.body.authed.RESERVED_USID)
  res.json(resMapper.filesView(files, req.body.authed))
}

async function downloadZip(req, res, next) {
  try {
    await fileSvc.streamZip(res, req.body.authed.RESERVED_USID, req.body.documents)
  } catch (err) {
...
async function retrieveFileById(req, res, next) {
  ...
  res.json(resMapper.filesView(file, req.body.authed))
}

async function retrieveAllVersions(req, res, next) {
  const versions = await fileSvc.retrieveAllVersions(req.params.accessId, req.body.authed.RESERVED_USID)
  res.json(resMapper.objReplaceKeyName(versions.reverse(), '_id', 'id'))
}

async function handleUpload(req, res, next) {
  var [file, errMsg] = await fileSvc.uploadFile(
    req.multerCustomHeader.RESERVED_USID,
  ...
  if (errMsg) {
    ...
    res.json(resMapper.filesView(file, req.multerCustomHeader))
  }
}
...
async function handleChecklistUpload(req, res, next) {
  ...
  await fileSvc.createFolder(req.multerCustomHeader.RESERVED_USID, constants.RESERVED_PATH.CHECKLIST, req.time)
  // get the documentType
  const documentType = req.params.documentType
  const existing = await fileSvc.findChecklistAccess(req.multerCustomHeader.RESERVED_USID, documentType)
...
async function modifyFileMetaThenResp(req, res, next) {
  ...
  if (errMsg) {
    ...
    res.json(resMapper.filesView(fileRes, req.body.authed))
  }
}
