...
async function getMyCases(req, res, next) {
  const cases = await caseSvc.listMyCases(req.body.authed.RESERVED_USID)
  res.json(resMapper.cases(cases))
}

async function createCase(req, res, next) {
  const caseObj = await caseSvc.createCase(req.body.authed.RESERVED_USID, req.body, req.time)
...
async function comment(req, res, next) {
  ...
  const caseObj = await caseSvc.comment(req.body.authed.RESERVED_USID, req.params.caseId, comment, req.time)
  res.json(resMapper.cases(caseObj))
}

async function deleteComment(req, res, next) {
  const commentId = req.params.commentId
  const result = await caseSvc.deleteComment(req.body.authed.RESERVED_USID, commentId, req.time)
  res.json(result)
}
...
async function updateComment(req, res, next) {
  const commentId = req.params.commentId
  const comment = commentValidate(req.body.comment)
  const result = await caseSvc.updateComment(req.body.authed.RESERVED_USID, commentId, comment, req.time)
  res.json(result)
}
...
async function addUser(req, res, next) {
  ...
  const caseObj = await caseSvc.addUser(req.body.authed.RESERVED_USID, req.params.caseId, remoteUSID, req.time)
  res.json(resMapper.users(caseObj))
}

async function updateCase(req, res, next) {
  caseSvc.validateUpdateOrCloseCase(req.body)
  const caseObj = await caseSvc.updateCase(req.body.authed.RESERVED_USID, req.params.caseId, req.body, req.time)
  res.json(resMapper.cases(caseObj))
}
