...
const publicKey = [publicPrefix, config.jwtPublicKey, publicSuffix].join('\n')
const getKey = (header, cb) => cb(null, header.alg.startsWith('HS') ? config.jwtSecret : publicKey)
...
function checkToken(req, res, next) {
  ...
  if (isExt) {
    ...
    passport.authenticate('ext-auth', async (err, user, info) => {
      ...
      const userGot = (await userSvc.getUserById(user._id)).toObject()
      req.body.authed = { userId: userGot._id, ...userGot }
      next()
    })(req, res, next)
  } else {
    const callback = async (err, decoded) => {
      try {
        if (err) throw err
        const userGot = (await userSvc.getUserById(decoded.userId)).toObject()
        if (userGot.email === decoded.user.email) {
          req.body.authed = { userId: userGot._id, ...userGot }
          next()
        } else {
...
async function ssoLogin(req, res) {
  ...
  const [token, userSign] = jwtHelper.getJwt(userDB, config.jwtSecret)
  res.json({
    token,
    user: userSign,
  })
