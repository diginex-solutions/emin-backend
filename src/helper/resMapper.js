...
const filesView = (fileResp, authedUser) => {
  if (Array.isArray(fileResp)) {
    fileResp = fileResp.map(f => formatOwner(f, authedUser.userId)).map(ethStatusMapping)
  } else {
    fileResp = ethStatusMapping(formatOwner(fileResp, authedUser.userId))
  }
...
const histories = (histories, authedUser) => {
...
const users = (users) => {
  const isList = Array.isArray(users)
  users = isList ? users : [users]
  users = users.map(u => {
    return {
      ...u,
      isExternal: u.applicationId ? true : false
    }
  })
  users = objReplaceKeyName(users, '_id', 'id')
  return isList ? users : users[0]
}
