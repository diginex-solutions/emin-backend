...
const createVerificationAccess = async (USID, myAccess, remoteUSID, message, ts, ipAddress, templateId, isVerify) => {
...
const completeVerification = async (userId, formId, statusU, inputs, ts, ipAddress, notes) => {
  ...
  if (status === STATUSES.rejected.text || status === STATUSES.accepted.text) {
    ...
    if (isCompleted) {
      ...
      shouldDo = !_.isEqual(answers, inputsObj) // frontend might send the same request, just skip doing changes, while we can't throw error
    }
    ...
    if (status !== STATUSES.received.text) {
      // frontend might make a verf/ques to `received` state even it's in received|rejected|accepted.
      // just don't throw error, and let it response.
