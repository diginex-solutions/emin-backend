...
export class CaseService {
  ...
  async deleteCaseType(spaceId: string, caseTypeId: string) {
    const deletedResult = await caseTypeModel.deleteOne({ _id: caseTypeId, spaceId })
    if (deletedResult.ok !== 1) {
      throw new cError.ResourceNotFoundException(`Case type not found with id: ${JSON.stringify({ spaceId, caseTypeId })}`)
    }
    return {
      deletedCount: deletedResult.deletedCount,
      success: deletedResult.deletedCount > 0
    }
  }
  ...
  async eventToResponse(event: CaseEvent): Promise<any> {
    ...
    const initiator = await userSvc.getUser(userIdInit)
    const recipientIdInit = event.recipientId ? await spaceSvc.findUserIdByUserSpace({_id: event.recipientId}) : undefined
    const recipient = recipientIdInit ? await userSvc.getUser(recipientIdInit) : undefined
  ...
  async addUser(USID: string, caseId: string, remoteUSID: string, ts: number) {
    ...
    const user = await userSvc.getUser(String(us.userId))
    const remoteUser = await userSvc.getUser(String(usRemote.userId))
    ...
    const link = `${config.emailFrontendDomain}/cases/${caseId}/feed?spaceId=${us.spaceId}`
