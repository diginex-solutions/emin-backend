...
const listChecklistAccesses = async (USID) => {
  ...
  for (let i = 0; i < accessesComplete.length; i++) {
    const access = accessesComplete[i];
    debugger
    let accepted = false;
    ...
    access.documentUploaded = true;
  }

  const existingTypes = accessesComplete.map((access) => access.category).filter((access) => !!access)
...
let uploadNewVersion = async (USID, versionFileId, accessId, size, storage, shaHash, ts, ipAddress) => {
  const myAccess = await retrieveMyAccessById(accessId, USID)
